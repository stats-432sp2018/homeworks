...
real.estate$Since1980 <- ifelse(real.estate$YearBuild > 1980, 1, 0)
```
...
all.the.formulas <- vector(length=32, mode="character")
...
default.terms <- c("factor(Quality)", "Sqft", "factor(Bedroom)",
            ...
            for (after.1980 in c(TRUE, FALSE)) {
                for (lot in c(TRUE, FALSE)) {
                    ...
                    if (recode.bedrooms) {
                        ...
                        the.terms <- union(the.terms, "factor(Beds.Coded)")
                        if (area.by.bedrooms) {
                            the.terms <- setdiff(the.terms,
                                                 "Sqft:factor(Bedroom)")
                            the.terms <- union(the.terms,
                                               "Sqft:factor(Beds.Coded)")
                        }
                    }
                    if (!area.by.bedrooms) {
...
# Using the due-date just for definiteness (trick from T. Lumley)
set.seed(2015-11-17)
...
second.best.index <- which.min(all.the.LOOCVs[-best.index])
```
...
best.formula <- formula(all.the.models[[best.index]])
...
added <- setdiff(best.terms, default.terms)
# What terms does it drop?
dropped <- setdiff(default.terms, best.terms)
# Similarly for the second-best model
second.best.formula <- formula(all.the.models[-best.index][[second.best.index]])
second.best.terms <- attr(terms(second.best.formula), "term.labels")
second.added <- setdiff(second.best.terms, default.terms)
second.dropped <- setdiff(default.terms, second.best.terms)
```
...
The client's other specific questions are best answered using figures.  Figure
7, left, shows the contribution made to the predicted price by the finished
...
Turning to the right panel of Figure 7, it shows younger houses are predicted
to have higher prices, with the premium on youth turning up fairly sharply if
the house was built after 1980.  This premium is, for the most, much reduced if
house is next to a highway, though the two lines coincide around,
coincidentally, 1980, and for very recent houses those next to highways are
predicted to be more expensive.  However, there are just
`r with(real.estate, sum(AdjHighway==1))` houses next to highways, with only
`r with(real.estate, sum(AdjHighway==1 & Since1980==1))` of those built since
1980.  The contrast here is necessarily very uncertain (and is missing from
the second-best model).




```{r, variable-slope-plots}
par(mfrow=c(1,2))
...
curve(x*favorite.coefficients["Sqft"],
      from=min(real.estate$Sqft), to=max(real.estate$Sqft),
      xlab="Finished area", ylab="Contribution to price",
      ylim=c(0,4e5))
rug(with(real.estate, Sqft[Beds.Coded=="1"]),side=1)
abline(a=favorite.coefficients["factor(Beds.Coded)2--4"],
       b=favorite.coefficients["Sqft"]+
         favorite.coefficients["Sqft:factor(Beds.Coded)2--4"], col="blue")
rug(with(real.estate, Sqft[Beds.Coded=="2--4"]),side=1, col="blue")
abline(a=favorite.coefficients["factor(Beds.Coded)5+"],
       b=favorite.coefficients["Sqft"]+
         favorite.coefficients["Sqft:factor(Beds.Coded)5+"], col="red")
rug(with(real.estate, Sqft[Beds.Coded=="5+"]),side=1, col="red")
legend("topleft", legend=paste("Bedrooms:", levels(real.estate$Beds.Coded)),
       col=c("black","blue","red"), lty="solid", cex=0.5)

# Year of construction
curve(x*favorite.coefficients["YearBuild"]
      + (x>1980)*favorite.coefficients["Since1980"]
      + x*(x>1980)*favorite.coefficients["YearBuild:Since1980"],
      from=min(real.estate$YearBuild), to=max(real.estate$YearBuild),
      xlab="Year of construction", ylab="Contribution to price",
      ylim=c(1.7e6,2.3e6))
rug(with(real.estate, YearBuild[AdjHighway==0]), side=1)
curve(favorite.coefficients["AdjHighway"]
      + x*favorite.coefficients["YearBuild"]
      + (x>1980)*favorite.coefficients["Since1980"]
      + x*(x>1980)*favorite.coefficients["YearBuild:Since1980"]
      + x*favorite.coefficients["YearBuild:AdjHighway"], add=TRUE,
      col="blue")
rug(with(real.estate, YearBuild[AdjHighway==1]), side=1, col="blue")
...
<!--
_Figure 7: The contribution of finished area to predicted
price, as a function of the number of bedrooms (left), and the contribution of
year of construction (relative to a baseline of the year 0, which is why the amounts are so huge), reflecting a changing trend after 1980 (right).  (The
estimate is surprisingly smooth, considering we didn't enforce
any sort of continuity.)  Rugs along the horizontal axis show the continuous
values belonging to each category._
-->
...
`r signif(sqrt(cv.lm(lm(formula(favorite.model),data=real.estate))),3)`$,
while that of our original model is
$\pm \$
`r signif(sqrt(cv.lm(initial.mdl)),3)`$.
Moreover, plotting residuals against fitted values shows almost exactly no
trend to the former, and reasonably constant variance (Figure 8, left).  (Plots
of residuals against predictors [omitted] are similarly good.)  But those same
residuals are still not very Gaussian (Figure 8, right).  Thus, while the
...
par(mfrow=c(1,2))
resid.vs.pred(favorite.model, "fitted", inference.set, pch=19, cex=0.5)
...
$`r signif(sqrt(cv.lm(lm(formula(favorite.model),data=real.estate))),2)`$.  The
