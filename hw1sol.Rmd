...
```{r geom_qq, echo=FALSE}
library(tidyverse)
## For some reason, ggplot doesn't like to draw the line
## the way that `qqline` does. Here's a function that helps.
## Running this will load one
...
***
With the addition of the interaction, we a change in vacancy rates will affect predicted rental rates through two sources: it's own marginal contribution and the interacted contribution with age. Holding age constant (non-zero), therefore, we have
\[
\Delta \widehat{\mbox{rent.rates}} = \widehat{\beta}_{\mbox{vac.rates}} + \widehat{\beta}_{\mbox{vac.rates}\times\mbox{age}} .
\]
So, in this case, for every 1% increase in vacancy rate, predicted rental rates increase by $`r coef(prop.lin.mod.interaction)['vac.rates']` + $`r coef(prop.lin.mod.interaction)['age:vac.rates']` = $`r coef(prop.lin.mod.interaction)['vac.rates']+coef(prop.lin.mod.interaction)['age:vac.rates']`
